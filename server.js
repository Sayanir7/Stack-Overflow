...
app.get('/home',authenticateJWT, async(req,res)=>{
    ...
    res.render('home', { questions, user:current_user });
...
app.get('/my-question', authenticateJWT, async (req, res) => {
    ...
    try {
        ...
        res.render('my-questions', { questionsWithAnswers ,answerWithQuestion, user: username});
    } catch (error) {
...
app.get('/question/:id', authenticateJWT,async (req, res) => {
    const question = await Question.findById(req.params.id);
    
    const answers = await Answer.find({ questionId: req.params.id }).sort({createdAt:-1});

    const current_user=req.cookies.username;
    res.render('question_detail', { question, answers , user: current_user});
});
...
app.post('/answer', authenticateJWT, async (req, res) => {
    ...
    if(current_user==user){
        ...
        res.status(400).json({ message: 'You cannot answer your own question' });
        return;
        //.json({ message: 'You cannot answer your own question' });
    }else{
        ...
        res.status(200).json({ redirectUrl: `/question/${questionId}` });
